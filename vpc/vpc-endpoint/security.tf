resource "aws_security_group" "allow_ssh_vpcA_regiao1" {
  depends_on = [ module.criar_vpcA_regiao1]

  provider = aws.primary

  vpc_id = module.criar_vpcA_regiao1.vpc_id
  description = "Regra para SSH e ICMP e HTTP e HTTPS"
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = [module.criar_vpcA_regiao1.cidr_block,module.criar_vpcB_regiao1.cidr_block]
  }

  ingress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [module.criar_vpcA_regiao1.cidr_block,module.criar_vpcB_regiao1.cidr_block]
  }

  ingress {
    from_port   = 8 # Código ICMP para Echo Request (ping)
    to_port     = 0 # ICMP não usa portas de destino, então configuramos como 0
    protocol    = "icmp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"          # "-1" representa todos os protocolos
    cidr_blocks = ["0.0.0.0/0"]
  }
}

resource "aws_security_group" "allow_ssh_vpcB_regiao1" {
  depends_on = [ module.criar_vpcB_regiao1]

  provider = aws.primary

  vpc_id = module.criar_vpcB_regiao1.vpc_id
  description = "Regra para SSH e ICMP e HTTP e HTTPS"
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = [module.criar_vpcB_regiao1.cidr_block]
  }

  ingress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [module.criar_vpcB_regiao1.cidr_block]
  }

  ingress {
    from_port   = 8 # Código ICMP para Echo Request (ping)
    to_port     = 0 # ICMP não usa portas de destino, então configuramos como 0
    protocol    = "icmp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"          # "-1" representa todos os protocolos
    cidr_blocks = ["0.0.0.0/0"]
  }
}



resource "aws_key_pair" "keyPairSSH_regiao_1" {
  key_name   = "minhaChaveSSH"
  public_key = "ssh-rsa  minhaChave" # Chave pública

  provider = aws.primary
}
