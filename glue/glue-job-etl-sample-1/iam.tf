...
# Permissao baixar modulos python
# https://docs.aws.amazon.com/pt_br/glue/latest/dg/getting-started-min-privs-job.html
data "aws_iam_policy_document" "glue_studio_transforms_read" {
  statement {
    sid     = "ListBucket"
    effect  = "Allow"
    actions = ["s3:ListBucket"]
    resources = ["arn:aws:s3:::aws-glue-studio-transforms-510798373988-prod-us-east-1"]
  }
...
resource "aws_iam_policy" "glue_studio_transforms_read" {
  name   = "glue-studio-transforms-read"
  policy = data.aws_iam_policy_document.glue_studio_transforms_read.json
}

resource "aws_iam_role_policy_attachment" "attach_glue_studio_transforms_read" {
  role       = aws_iam_role.glue_role.name
  policy_arn = aws_iam_policy.glue_studio_transforms_read.arn
}