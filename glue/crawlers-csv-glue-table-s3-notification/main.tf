...
resource "aws_glue_crawler" "customers_crawler" {
  ...
  s3_target {
    ...
    event_queue_arn = aws_sqs_queue.s3_sqs_events.arn
  }
  ...
  # roda em agenda; em cada run ele consome eventos da SQS (se não tiver evento, ele termina rápido)
  schedule = "cron(0/15 * * * ? *)" # a cada 15 min

  depends_on = [aws_s3_bucket_notification.to_sqs]
}
